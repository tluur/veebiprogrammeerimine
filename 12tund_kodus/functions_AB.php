...
function userId_FilmId(){
	$notice_ufID = null;
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("INSERT INTO film_ja_user_id (userid) VALUES(?)");
	echo $conn->error;
	$stmt->bind_param("i", $_SESSION["userID"] );
				if($stmt->execute()){
	$notice_ufID = "Info on salveststudf";
	} else {
	$notice_ufID = "ei ole salvestatud" .$stmt->error;
